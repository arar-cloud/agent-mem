...
## Performance Note: Text Extraction Optimization

The session management functions (basicSession, multiTurn, sessionResume) extract assistant text messages from async message streams. To avoid redundant filtering and type-checking on every message, text extraction is centralized into a reusable utility function:

```typescript
// Utility: Extract assistant text message from message array
function extractAssistantText(messages: Message[]): string | undefined {
  return messages.find(m => m.role === 'assistant' && m.type === 'text')?.content;
}
```

This single function is used across all session handlers instead of repeating the find()+type guard pattern, reducing CPU overhead and improving iteration performance.
